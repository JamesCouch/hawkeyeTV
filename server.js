...
db.run("CREATE TABLE IF NOT EXISTS profiles (id INTEGER PRIMARY KEY, zipcode TEXT, theme TEXT, facebook TEXT, twitter TEXT, news TEXT)");
...
db.serialize(function(){
    db.get("SELECT * FROM profiles WHERE id = ?", [ "1" ], function(err, profile){
        if(profile){
            ...
            db.run("INSERT INTO profiles (zipcode, theme, facebook, twitter, news) VALUES (?,?,?,?,?)", [ "52242", "Default", "Disabled", "Disabled", "Disabled"], function(err){ 
                if(err){
                    console.log(err);
                }
            });
...
app.post("/api/auth/update", function(req, res){
    db.serialize(function(){
        ...
        db.run("UPDATE profiles SET zipcode = ?, theme =?, facebook = ?, twitter = ?, news = ? WHERE id = ?", [ req.body.zipcode, req.body.theme, req.body.facebook, req.body.twitter, req.body.news, req.body.id ], function(err){
            if(err) {
